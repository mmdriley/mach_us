 ...
 */
/*
 * File: $Source: /afs/cs.cmu.edu/project/mach/mach3/rcs/us/server/pipenet/pipenet_clts_recs_ifc.h,v $
 *
 ...
 *
 * HISTORY
 * $Log:	pipenet_clts_recs_ifc.h,v $
 * Revision 2.4  94/07/13  17:21:02  mrt
 * 	Updated copyright
 * 
 * Revision 2.3  91/11/06  14:20:25  jms
 * 	Initial C++ revision.
 * 	[91/09/27  15:52:21  pjg]
 * 
 * Revision 2.2  91/05/05  19:32:35  dpj
 * 	Merged up to US39
...
#ifndef	_pipenet_clts_recs_ifc_h
#define	_pipenet_clts_recs_ifc_h

#include	<pipenet_recio_ifc.h>
#include	<pipenet_clts_base_ifc.h>

class pipenet_clts_recs: public pipenet_clts_base, public pipenet_recio {
      public:
	DECLARE_MEMBERS(pipenet_clts_recs);
	virtual char* remote_class_name() const;

	pipenet_clts_recs(ns_mgr_id_t =null_mgr_id, access_table * =0,
			  pipenet_dir_base * =0, net_addr_t * =0,
			  default_iobuf_mgr * =0);

REMOTE	virtual mach_error_t net_clts_read1rec(io_mode_t, char *, 
...
REMOTE	virtual mach_error_t ns_get_attributes(ns_attr_t, int *);
	...
	virtual mach_error_t io_write1rec(io_mode_t, io_recnum_t, char*,
					  unsigned int);
	virtual mach_error_t io_read1rec_seq(io_mode_t, char*, unsigned int*,
					     io_recnum_t*);
	virtual mach_error_t io_write1rec_seq(io_mode_t, char*, unsigned int,
					      io_recnum_t*);
...
#endif	_pipenet_clts_recs_ifc_h
